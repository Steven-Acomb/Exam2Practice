...
class Box(object):
    ...
    def append_string(self, additional_contents):
        ...
        #    and continue working on the problem.
        # --------------------------------------------------------------
        extra_space = self.volume - len(self.contents)
        overflow = len(additional_contents) - extra_space
        remainder = ''
        if(overflow > 0):
            for k in range(extra_space):
                self.contents += (additional_contents[k])
            for j in range(overflow):
                remainder += additional_contents[extra_space+j]
        else:
    ...
    def double_then_shrink(self, new_volume):
        ...
        # --------------------------------------------------------------
        sum = len(self.double())
        sum += len(self.shrink(new_volume))
        return sum
    ...
    def combined_box(self, other_box):
        ...
        #    TIME ESTIMATE:   5 minutes.
        # --------------------------------------------------------------
        box = Box(self.contents+other_box.contents,
